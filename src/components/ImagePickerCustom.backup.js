...
import { IMAGE_PLACEHOLDER_BASE64 } from '../utils/placeholders';
...
const ImagePickerCustom = ({ 
  ...
  const handleOpenCamera = async () => {
    try {
      console.log('[ImagePicker] Solicitando permisos de cámara...');
      const { status } = await ImagePicker.requestCameraPermissionsAsync();
      
      if (status !== 'granted') {
        console.log('[ImagePicker] Permiso de cámara denegado');
        Alert.alert('Permiso denegado', 'Necesitamos acceso a la cámara.');
        return;
      }
      
      console.log('[ImagePicker] Abriendo cámara...');
      const result = await ImagePicker.launchCameraAsync({
        mediaTypes: ImagePicker.MediaTypeOptions.Images,
        allowsEditing: true, // Permitir editar para recortar/ajustar la imagen
        aspect: [aspectRatio[0], aspectRatio[1]], // Usar la misma relación de aspecto
        quality: 0.8,
      });
      
      console.log('[ImagePicker] Resultado de cámara:', result);
      
      if (!result.canceled && result.assets && result.assets.length > 0) {
        const capturedImage = result.assets[0];
        
        // Crear un objeto con el mismo formato que las imágenes de galería
        const normalizedImage = {
          id: 'captured_' + Date.now(),
          uri: capturedImage.uri,
          width: capturedImage.width || 800,
          height: capturedImage.height || 1000,
        ...
        console.log('[ImagePicker] Imagen capturada normalizada:', normalizedImage);
        
        // Actualizar la selección con los datos normalizados
        setSelectedImage(normalizedImage);
        
        // Refrescar la galería para que aparezca la nueva foto (en segundo plano)
        // pero no esperamos a que termine para mejorar la experiencia
        loadPhotosFromGallery().catch(err => 
          console.warn('[ImagePicker] Error al refrescar galería tras captura:', err)
        );
      }
    } catch (error) {
  ...
  // Manejar la selección de una imagen y asegurar que siempre se previsualice antes
  const handleSelectPhoto = (photo) => {
    ...
    console.log(`[ImagePicker] Imagen seleccionada de galería:`, photo);
    
    // Verificar que la URI es válida
    if (!photo.uri || typeof photo.uri !== 'string') {
      console.error('[ImagePicker] URI inválida:', photo);
      Alert.alert('Error', 'La imagen seleccionada no es válida.');
      return;
    }
    
    // Crear una copia normalizada de la imagen seleccionada
    // Esto asegura que todas las imágenes, sea cual sea su origen,
    // tengan la misma estructura
    const normalizedPhoto = {
      id: photo.id || `gallery_${Date.now()}`,
      uri: photo.uri,
      width: photo.width || 800,
      height: photo.height || 1000,
      aspectRatio: aspectRatio,
      previewLoaded: true,  // Marcar como previamente visualizada
      _timestamp: Date.now()
    };
    
    console.log(`[ImagePicker] Imagen normalizada:`, normalizedPhoto);
    
    // Actualizar la selección para mostrar en la previsualización
    setSelectedImage(normalizedPhoto);
  };
  ...
  const handleNext = () => {
    ...
    if (selectedImage && selectedImage.id !== 'camera') {
      ...
      // Verificar si la imagen ha sido previamente visualizada
      if (!selectedImage.previewLoaded) {
        console.warn('[ImagePicker] Intento de seleccionar imagen sin previsualización previa');
        Alert.alert(
          'Espera un momento', 
          'La imagen aún se está cargando. Por favor, espera a que se visualice correctamente.',
          [{ text: 'OK' }]
        );
        return;
      }
        ...
        {isCamera ? (
          ...
          <>
            <Image 
              ...
              resizeMode="cover"
              // Usar el placeholder en base64 como fallback
              defaultSource={{uri: IMAGE_PLACEHOLDER_BASE64}}
              // Mejorar el manejo de errores y carga
              onError={(e) => {
                console.error(`[ImagePicker] Error cargando miniatura: ${item.id}`, e.nativeEvent.error);
                console.error(`[ImagePicker] URI problemática: ${item.uri}`);
              }}
              onLoadEnd={() => {
                console.log(`[ImagePicker] Miniatura cargada correctamente: ${item.id}`);
                // Si esta es la imagen actualmente seleccionada, marcamos que se ha previsualizado
          ...
          {loading ? (
            ...
            <>
              <Image 
                ...
                // Usar el placeholder en base64 como fallback
                defaultSource={{uri: IMAGE_PLACEHOLDER_BASE64}}
                // Verificamos errores de carga
                onError={(e) => {
                  console.error('[ImagePicker] Error cargando imagen en visor:', e.nativeEvent.error);
                  console.error('[ImagePicker] URI problemática:', selectedImage.uri);
                  Alert.alert('Error', 'No se pudo cargar la imagen seleccionada. Por favor, selecciona otra.');
                }}
                // Marcamos cuando se carga correctamente
                onLoadEnd={() => {
                  console.log('[ImagePicker] Imagen cargada en previsualizador:', selectedImage.id);
                  // Marcar explícitamente que la imagen se ha previsualizado correctamente
                  setSelectedImage(prev => ({
                    ...prev,
                    previewLoaded: true
                  }));
